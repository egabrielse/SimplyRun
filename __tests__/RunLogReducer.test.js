...
describe('RunLogReducer', () => {
    ...
    it('should add a new run entry to the runs store when given a ADD_RUN action with valid run data', () => {
    ...
    it('should delete a run entry from the run store when given a DELETE_RUN action with a valid run id', () => {
    ...
    // TEST 5: NON EMPTY STATE + DELETE_RUN
    it('should return state unchanged when given a DELETE_RUN action with an invalid run id', () => {
        expect(RunLogReducer(nonEmptyState, {type:DELETE_RUN, id: "non-existant-id",
        })).toEqual({
            runs:[{id: "A0mIWhNc74WVyIaeQxXg", time: 720, distance: 2.1, pace: 5.71428571, calories: 475},
            {id: "X7dIhDstHGkoWLqyAjcv", time: 2420, distance: 4.9, pace: 8.23129252, calories: 750},
            {id: "2FMpFLsVueJHv24wOxky", time: 600, distance: 1.1, pace: 9.09090909, calories: 530},
